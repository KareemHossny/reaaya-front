...
// Helper to format numbers with commas for statistics
...
const DoctorStats = () => {
  ...
  // Cards content (+ percentages/trends as placeholder, can update with backend if data added)
    ...
    () => [
      {
        ...
        trend: statsSafe.trendTotal !== undefined ? statsSafe.trendTotal : '+9%',
        percent: 100
      },
      {
        title: t('today_appointments_stat'),
        value: statsSafe.todayAppointments,
        color: 'from-green-500 to-green-700',
        icon: CalendarIcon,
        trend: t('today'),
        percent: total ? Math.round((statsSafe.todayAppointments / total) * 100) : 0
      },
      {
  ...
  // Doctor Tips - make them translatable
  const tips = useMemo(
    () => [
      t('update_availability_regularly'),
      t('inform_patients_changes'),
      t('review_today_morning'),
      t('use_cancel_emergency_only')
    ],
    [t]
  );
  ...
  const completedRate = useMemo(
    () =>
      total
        ? Math.round(((statsSafe.completedAppointments || 0) / total) * 100)
        : 0,
  ...
  const cancelledRate = useMemo(
    () =>
      total
        ? Math.round(((statsSafe.cancelledAppointments || 0) / total) * 100)
        : 0,
  ...
  const attendanceRate = useMemo(
    () =>
      total
        ? Math.round(
            ((total - (statsSafe.cancelledAppointments || 0)) / total) * 100
          )
        : 0,
    [statsSafe.cancelledAppointments, total]
  );

  // Loader UI (match patient)
  if (loading) {
    return (
      <div
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4 xs:gap-7">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden flex flex-col h-full min-w-0">
          <div className="bg-gradient-to-r from-green-500 to-green-600 p-4 xs:p-5 text-white flex items-center gap-2 xs:gap-3">
            ...
            <h3 className="text-base xs:text-lg md:text-xl font-bold">{t('doctor_tips')}</h3>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden flex flex-col h-full min-w-0">
          <div className="bg-gradient-to-r from-purple-500 to-purple-600 p-4 xs:p-5 text-white flex items-center gap-2 xs:gap-3">
            ...
            <h3 className="text-base xs:text-lg md:text-xl font-bold">{t('clinic_insights')}</h3>
          </div>
          <div className="p-4 xs:p-6 flex-1">
            <div className="space-y-3 xs:space-y-4">
              ...
              <div className="flex flex-row items-center justify-between p-2 xs:p-3 bg-green-50 rounded-xl">
                <span className="text-gray-700 font-medium text-xs xs:text-base">{t('attendance_rate')}</span>
                <span className="text-xl xs:text-2xl font-bold text-green-600 tabular-nums">
                  {attendanceRate}%
                </span>
              </div>
...
export default React.memo(DoctorStats);